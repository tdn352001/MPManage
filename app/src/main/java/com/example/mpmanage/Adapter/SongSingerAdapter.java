...
public class SongSingerAdapter extends RecyclerView.Adapter<SongSingerAdapter.ViewHolder> implements Filterable {
    UpdateSongActivity context;
    ArrayList<CaSi> arrayList;
    ArrayList<CaSi> mArrayList;
    boolean isDialog;

    public SongSingerAdapter(UpdateSongActivity context, ArrayList<CaSi> arrayList) {
        ...
        if (!isDialog) {
                ...
                if (arrayList.size() == 1) {
                    ...
                    for(int i = 0; i < context.caSiArrayList.size(); i++){
                        if(context.caSiArrayList.get(i).getIdCaSi().equals(caSi.getIdCaSi())){
                            ...
                            context.songSingerAdapter.notifyItemRemoved(i);
                            break;
                        }
                    }
            ...
            holder.itemView.setOnClickListener(v -> {
                if (context.caSiArrayList.contains(caSi)) {
                    ...
                    context.songSingerAdapter.notifyItemInserted(context.caSiArrayList.size());
